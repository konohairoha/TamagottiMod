...
import tamagotti.init.tile.model.ModelWindMill;

public class RenderTileWindMill extends TileEntitySpecialRenderer<TileWindMill> {

	float angle = 0;

	private final static  ResourceLocation TEX = new ResourceLocation("tamagottimod:textures/entity/windmill_brass.png");

	@Override
	public void render(TileWindMill te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {


		ModelWindMill model = this.getModel(te);
	...
	protected ModelWindMill getModel(TileWindMill te) {
		return new ModelWindMill();
	}

	public void render(TileWindMill te, ModelWindMill model, float rot, float speed, float tick) {
