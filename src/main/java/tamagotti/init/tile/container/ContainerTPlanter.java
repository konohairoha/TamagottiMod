...
public class ContainerTPlanter extends Container {

	final TileTPlanter tile;

	public ContainerTPlanter(InventoryPlayer invPlayer, TileTPlanter tile) {
		this.tile = tile;
	...
	void initSlots(InventoryPlayer invPlayer) {
		...
		for (int i = 0; i < 6; i++)
			for (int j = 0; j < 6; j++)
				this.addSlotToContainer(new ValidatedSlot(output, i + j * 6, 62 + i * 18, 6 + j * 18, SlotPredicates.PLANT));
